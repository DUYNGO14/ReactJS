...
const Pagination : React.FC<Pagination> = ({ page, per_page, total, total_pages, setPage,  }) => {
    return (
        <Box className="max-w-screen-xl mx-auto mt-12 px-4 text-gray-600 md:px-8">
            <Box className="hidden justify-between text-sm md:flex">
                <Box>
                    SHOWING {(page - 1) * per_page + 1} - {page * per_page > total ? total : page * per_page} OF {total}
                </Box>
                <Box className="flex items-center gap-12" aria-label="Pagination">
                    <Button 
                     onClick={() => page > 1 && setPage(page - 1)} 
                     className="hover:text-indigo-600 text-lg border border-indigo-600 rounded-lg" ><TiChevronLeft /></Button>
                    ...
                    className="hover:text-indigo-600 text-lg border border-indigo-600 rounded-lg"><TiChevronRight /></Button>
                </Box>
            </Box>
            {/* On mobile version */}
            <Box className="flex items-center justify-between text-sm text-gray-600 font-medium md:hidden">
                <Button 
                     onClick={() => page > 1 && setPage(page - 1)} 
                     className="hover:text-indigo-600 text-lg border border-indigo-600 rounded-lg" ><TiChevronLeft />
                </Button>
                ...
                <Button
                    onClick={() => page < total_pages && setPage(page + 1)} 
                    className="hover:text-indigo-600 text-lg border border-indigo-600 rounded-lg"><TiChevronRight />
                </Button>
            </Box>
        </Box>
    )
